...
// Default FAQ data as a fallback (an array with a single FAQ group)
const defaultFaq = [
  {
...
export default function FAQ() {
  ...
  // Determine which FAQ group to render: if API data is available, use the first group; otherwise, use our default group.
  const faqGroup = useMemo(() => {
    if (
      status === "success" &&
      homePageData?.FAQ &&
      homePageData.FAQ.length > 0
    ) {
      return homePageData.FAQ[0];
    }
    return defaultFaq[0];
  }, [homePageData, status]);
  ...
  return (
    <div className="py-24 px-2 md:px-20">
      <h1 className="font-extrabold text-center text-4xl mb-10 text-[#003956] md:text-5xl leading-[50px]">
        {faqGroup.title}
      </h1>
      <div className="flex flex-col items-center justify-center text-center gap-5">
        {faqGroup.faq.slice(0, showMore).map((item, i) => (
          <FaqCard key={i} {...item} i={i} />
        ))}
      </div>
      {showMore < faqGroup.faq.length && (
        <button
